...
from common import find_closest_zone
from control.controller import Controller
from game.game import Game
from logger import Colors, get_logger, debug_coro
from vision.vision import Vision

logger = get_logger('loldroid.Bot')
...
class LolDroid:
    ...
    def run(self):
        logger.info(f'Welcome to {Colors.yellow}LolDroid{Colors.reset}')
        self.build_client()
    ...
    def build_client(self):
        logger.info(f'Retrieving {Colors.yellow}client data:{Colors.reset}')
        self.client_data = ClientData()
        logger.info(f'Finished retrieving {Colors.green}client data{Colors.reset}')

        logger.info(f'Initializing {Colors.yellow}LoLDroid client:{Colors.reset}')
        self.client = Client(client_data=self.client_data)
        print(self.client)
        time.sleep(20)
